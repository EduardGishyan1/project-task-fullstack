...
const ProductFunctionality = () => {
  ...
  const handleAddProduct = async () => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:4021/products", {
        method: "POST",
        headers: {
  ...
  const handleDeleteProduct = async () => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:4021/products/${productId}`, {
        method: "DELETE",
      });
      if (response.ok) {
  ...
  const handleUpdateProduct = async () => {
    if (!productId || !updatedName || !updatedPrice || !updatedStock || !updatedDescription || !updatedCategory) {
      alert("Please fill in all fields.");
      return;
    }
    try {
